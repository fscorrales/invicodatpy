...
class ResumenRecaudado(RPWUtils):
    ...
    _REPORT_TITLE:str = field(
        ...
        default='Resumen Recaudado'
    )
    _TABLE_NAME:str = field(
        init=False, repr=False, 
        default='resumen_recaudado'
    )
   ...
   # --------------------------------------------------
    def download_report(
        ...
        try:
            ...
            self.sgv.driver.get('http://ipvsql.invico.gov.ar/reports/report/Publicos/Financiera/InformeResumenRecaudado')

            time.sleep(20)

            # xpath_input_gral = "//table[@class='tablaFiltros']//input"
            xpath_ejercicio = "//input[@id='ReportViewerControl_ctl04_ctl05_txtValue']"
            # xpath_ejercicio = "/html/body/form/table/tbody/tr/td/div[2]/div/table/tbody/tr[2]/td/div/div/table/tbody/tr/td[1]/table/tbody/tr/td[5]/div/input"
            # self.sgv.wait.until(EC.presence_of_element_located(
            #     (By.XPATH, xpath_ejercicio)
            # ))
            # Llenado de inputs
            # input_elements = self.sgv.driver.find_elements(By.TAG_NAME, "input")
            # for input_element in input_elements:
            #     print("Tag:", input_element.tag_name)
            #     print("Tipo:", input_element.get_attribute("type"))
            #     print("Nombre:", input_element.get_attribute("name"))
            #     print("ID:", input_element.get_attribute("id"))
            #     print("Clases:", input_element.get_attribute("class"))
            #     print("Valor:", input_element.get_attribute("value"))
            #     print("Texto visible:", input_element.text)
            #     print("-" * 50)
            cmb_tipo_vivienda = Select(
                self.sgv.driver.find_element(By.XPATH, "//select[@id='ReportViewerControl_ctl04_ctl03_ddValue']")
            )
            cmb_tipo_vivienda.select_by_visible_text('Todos los Rubros')
            ...
            for ejercicio in ejercicios:
                ...
                if int_ejercicio > 2010 and int_ejercicio < (dt.datetime.now().year + 1):
                    ...
                    self.sgv.wait.until(EC.presence_of_element_located(
                        (By.XPATH, "/html/body/form/table/tbody/tr/td/div[2]/div/table/tbody/tr[5]/td[3]/div/div[1]/div/table/tbody/tr/td/table/tbody/tr[1]/td/table/tbody/tr/td/table'")
                    ))
                    btn_export = self.sgv.driver.find_element(By.XPATH, "//a[@id='ReportViewerControl_ctl05_ctl04_ctl00_ButtonLink']")
                    btn_export.click()
                    time.sleep(1)
    ...
    if args.download:
        ...
        if args.username != '' and args.password != '':
            ...
            if os.path.isfile(json_path):
                ...
                json_file.close()
        sgv = ResumenRecaudado(sgv = sgv_connection)
        sgv.download_report(
